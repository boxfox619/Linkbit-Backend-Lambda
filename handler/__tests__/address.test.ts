...
describe('get link address handler', () => {
    ...
    it('should return status code 400 with error', async () => {
        const event: APIGatewayProxyEvent = apiGatewayEventMock();
        const res = await addressHanders.linkAddress(event, contextMock(), () => { });
        expect(res).toBeDefined();
        const response = res as APIGatewayProxyResult;
        const body = JSON.parse(response.body);
        expect(response.statusCode).toBe(400);
    ...
    it('should return status code 200 and database update', async () => {
        ...
        const response = res as APIGatewayProxyResult;
        expect(response.statusCode).toBe(200);
        expect(addressRepo.linkAddress.getCalls().length).toBe(1);
        const linkAddressCall = addressRepo.linkAddress.getCall(0);
        expect(linkAddressCall.args[0]).toBe(linkaddress);
        expect(linkAddressCall.args[1]).toBe(accountaddress);
        expect(linkAddressCall.args[2]).toBe(symbol);
    });
});